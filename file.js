...
async function getFileData() {
    ...
    if (courseId) {
        try {
            const response = await fetch('https://zamayllinux.azurewebsites.net/api/files/' + courseId);
    ...
    try {
        ...
        if (dataPost.deleted) {
            const deleteUrl = dataPost.deleted === 'upVote'
                ? `https://zamayllinux.azurewebsites.net/api/FileUpVote/${dataPost.fileId}/upvote`
                : `https://zamayllinux.azurewebsites.net/api/FileDownVote/${dataPost.fileId}/downvote`;
...
let upvotes = 0;
        ...
        try {
            const response = await fetch(`https://zamayllinux.azurewebsites.net/api/FileDownVote/${dataPost.fileId}/downvotes/count`);
            if (!response.ok) {
                return true;
            }
        ...
        try {
            const response = await fetch(`https://zamayllinux.azurewebsites.net/api/FileUpVote/${dataPost.fileId}/upvotes/count`);
            if (!response.ok) {
                return true;
            }
            const responses = await response.json();;
